# 🚀 量子力学课程演示页面 

这是一个用于辅助南京航空航天大学(NUAA)物理学院《量子力学》教学的交互式可视化工具库。

---

### 🔗 核心演示模块 (Core Demonstrations)

以下是本仓库包含的主要演示页面：

*   **📝 课堂随机点名系统**
    *   [点击进入点名页面](https://wenlong-you.github.io/QuantumMechanics/dianming.html)
*   **🪜 一维无穷深势阱 (One-Dimensional Infinite Square Well)**
    *   [点击进入演示](https://wenlong-you.github.io/QuantumMechanics/1DInfinitewell.html)
*   **🕳️ 一维有限深势阱 (One-Dimensional Finite Square Well)**
    *   [点击进入演示](https://wenlong-you.github.io/QuantumMechanics/1Dfinitewell.html)
*   **🚧 一维势垒的隧穿 (One-Dimensional Potential Barrier Tunneling)**
    *   [点击进入演示](https://wenlong-you.github.io/QuantumMechanics/1Dpotentialbarrier.html)
*   **🎵 一维量子谐振子 (One-Dimensional Harmonic Oscillator)**
    *   [点击进入演示](1Dharmonicoscillator.html)
*   **📏 氢原子径向概率分布 (Hydrogen Atom Radial Probability Distribution)**
    *   [点击进入演示](https://wenlong-you.github.io/QuantumMechanics/H_Atom_RadialWF.html)
*   **🌐 氢原子角向概率密度 (Hydrogen Atom Angular Probability Density)**
    *   [点击进入演示](https://wenlong-you.github.io/QuantumMechanics/H_Atom_SphericalHarmonics.html)

📝 配套练习模块 (Practice)

🧪 氢原子波函数与径向概率分布测试
    简介：包含关于径向波函数 R(r)R(r) 、角向波函数 Y(θ,ϕ)Y(θ,ϕ) 以及径向概率分布 w(r)w(r) 的基础概念测试。
    重点题目：例如“径向概率分布函数 w(r)w(r) 与径向波函数 R(r)R(r) 的关系是？” ( w(r)=r2∣R(r)∣2w(r)=r2∣R(r)∣2 )
        
> **💡 提示 (Tip):** 
> 点击上述链接即可在浏览器中直接运行演示，无需安装任何软件。
